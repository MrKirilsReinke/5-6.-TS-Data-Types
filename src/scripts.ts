...
export interface Product {
  ...
  function filterCourses(products: Product[] /* add type annotation here */): Product[] /* add type annotation here */ {
    return products.filter(product => product.type === 'Course')
  }
  
  function getTitles(products: Product[] /* add type annotation here */): string[] /* add type annotation here */ {
  ...
  const coursetitles: string[] = getTitles(products);
  ...
  const price1: string = formatPrice(products[0]);
...
//   Everyday Types
  ...
  const anyProduct: Product1 = {id: 1, name: 'Mr. Fluff', kind: 'British Shorthair', age: 4, title: 'anyProduct', price: 1, price1}
  const productTitle = anyProduct.title
  const priceWithTaxes = anyProduct.price * (1.25)
  const upperCaseTitle = anyProduct.price1.toUpperCase();
  ...
  const getTax = (course: Course): number => {
  ...
  //  Why?
  const getCurrency = (account: {name: string, currency: string}): string => account.currency
